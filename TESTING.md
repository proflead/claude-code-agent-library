# Testing Guide for Claude Code Subagents
...
# Verify agents loaded by opening Claude Code and running /agents
```

### IDE Testing Prerequisites

**VS Code:**
1. Install Claude Code IDE integration (latest)
2. Reload VS Code window
3. Create test workspace
4. Copy agents to `.claude/agents/` in workspace
5. Reload window again

**Verification:**
- Open Claude Code (`Ctrl/Cmd + I`)
...
```bash
# CLI
claude --agent orchestrator "Build a todo list feature with:
...
```bash
# CLI
claude --agent orchestrator "Build authentication system with OAuth, JWT, email verification, and rate limiting"
...
```bash
# CLI
claude --agent workflow-manager "Create GitHub Actions workflow:
1. Run tests
...
```bash
# CLI
claude --agent project-manager "Plan 2-week sprint for user profile feature:
- Profile editing
- Avatar upload
- Privacy settings
- Activity feed
...
```bash
# CLI
claude --agent frontend-developer "Create React TypeScript component for user profile card with:
- Avatar, name, bio
...
```bash
# CLI
claude --agent backend-developer "Create Express.js API for blog posts:
- CRUD operations
...
```bash
# CLI
claude --agent fullstack-expert "Build real-time notifications:
...
```bash
# CLI
claude --agent mobile-developer "Create React Native camera feature:
...
```bash
# CLI
claude --agent api-designer "Design REST API for e-commerce:
...
```bash
# CLI
claude --agent database-architect "Design PostgreSQL schema for project management:
...
**Python Expert:**
```bash
claude --agent python-expert "Create async function to scrape website with:
...
**JavaScript Expert:**
```bash
claude --agent javascript-expert "Create custom Promise utility with:
...
**Rust Expert:**
```bash
claude --agent rust-expert "Create thread-safe LRU cache:
...
**Go Expert:**
```bash
claude --agent go-expert "Create worker pool:
...
**Java Expert:**
```bash
claude --agent java-expert "Create Spring Boot service:
- REST endpoints
- JPA entities
...
**SQL Expert:**
```bash
claude --agent sql-expert "Optimize slow query:
...
```bash
# CLI
claude --agent e2e-tester "Create Playwright tests for checkout:
...
```bash
# CLI
claude --agent ab-test-ideas "Generate A/B test ideas to improve:
...
```bash
# CLI
claude --agent code-reviewer "Review this authentication code:
...
```bash
# CLI
claude --agent security-auditor "Audit this API for OWASP Top 10:
[paste API code]"
...
```bash
# CLI
claude --agent test-generator "Create Jest tests for OrderService:
...
```bash
# CLI
claude --agent uiux-designer "Design onboarding flow for project management app:
...
```bash
# CLI
claude --agent figma-to-html "Convert landing page design:
...
```bash
# CLI
claude --agent responsive-design "Make this dashboard responsive:
...
```bash
# CLI
claude --agent design-system "Create design system for SaaS app:
...
```bash
# CLI
claude --agent enhanced-planner "Plan migration from monolith to microservices:
...
```bash
# CLI
claude --agent research-agent "Research React vs Vue vs Svelte for our next project:
- Learning curve
- Ecosystem maturity
- Performance
- Team size: 5 devs
...
```bash
# CLI
claude --agent task-breakdown "Break down epic: User Authentication
...
```bash
# CLI
claude --agent doc-generator "Create README for REST API:
...
```bash
# CLI
claude --agent docker-expert "Create Dockerfile for Node.js app:
- Multi-stage build
- Security hardening
- Minimize image size
- Non-root user
...
```bash
# CLI
claude --agent kubernetes-expert "Create K8s manifests for web app:
...
```bash
# CLI
claude --agent cicd-expert "Create GitHub Actions workflow:
- Run tests on PR
...
```bash
# CLI
claude --agent terraform-expert "Create Terraform for AWS:
...
```bash
# CLI
claude --agent debug-detective "Analyze error:
...
```bash
# CLI
claude --agent performance-optimizer "Optimize slow API:
...
```bash
# CLI
claude --agent legacy-modernizer "Plan migration:
From: AngularJS 1.5
To: React 18
