# Contributing to Claude Code Agent Library
...
- `model`: Recommended model (e.g., `sonnet`, `sonnet`)
...
- `.claude/agents/orchestration/` - Multi-agent coordination
- `.claude/agents/fullstack/` - Full-stack development
- `.claude/agents/coding/` - Language-specific experts
- `.claude/agents/testing/` - Testing and quality
- `.claude/agents/design/` - Design and UI/UX
- `.claude/agents/productivity/` - Planning and documentation
- `.claude/agents/devops/` - DevOps and infrastructure
- `.claude/agents/debugging/` - Debugging and optimization
...
```bash
cp templates/agent-template.md .claude/agents/[category]/[agent-name].md
```
...
description: Use when you need help doing amazing things efficiently.
model: sonnet
...
cp .claude/agents/[category]/[agent-name].md ~/.claude/agents/

# Test with various prompts
claude --agent [agent-name] "Test prompt 1"
claude --agent [agent-name] "Test prompt 2"
```

#### IDE Testing

1. Add agent to `.claude/agents/` in your workspace
...
```markdown
When backend implementation is needed, explicitly ask Claude Code to use the backend-developer subagent and include the API requirements, constraints, and expected outputs.
```
...
Thank you for contributing to Claude Code Agent Library! Your contributions help developers worldwide build better software faster.
