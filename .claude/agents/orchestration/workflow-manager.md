...
description: Use when you need manages sequential and parallel task execution with dependency resolution.
model: sonnet
...
claude --agent workflow-manager "Create a deployment workflow: run tests, build Docker image, push to registry, deploy to staging, run smoke tests"

# Example: Data pipeline
claude --agent workflow-manager "Design ETL workflow: extract from 3 sources in parallel, transform data, load to warehouse, update indexes"
```
