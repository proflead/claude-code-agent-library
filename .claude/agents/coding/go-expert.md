---
name: go-expert
description: Use when you need go specialist with concurrency patterns, error handling, and idiomatic Go practices.
model: sonnet
...
```bash
claude --agent go-expert "Create a worker pool pattern for concurrent job processing"
claude --agent go-expert "Build a middleware chain for HTTP request handling"
```
