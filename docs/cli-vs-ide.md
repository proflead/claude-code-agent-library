...
Claude Code subagents work in both the CLI and IDEs (like VS Code), but each platform has unique strengths. This guide helps you choose the right platform and use agents effectively in each environment.
...
| **Agent Invocation** | `claude --agent agent-name "task"` | `@agent-name task` in chat |
...
```bash
# Good - explicit paths
claude --agent python-expert "Review the user authentication in src/auth/login.py"

# Less clear - ambiguous
claude --agent python-expert "Review the login file"
```
...
```bash
# Save generated code
claude --agent backend-developer "Create user repository class" > src/repositories/user.py

# Save documentation
claude --agent doc-generator "Create README" > README.md

# Append to existing file
claude --agent test-generator "Add tests for UserService" >> tests/test_user_service.py
```
...
claude --agent research-agent "Research FastAPI best practices" > research.md

# Implement based on research
claude --agent backend-developer "Create FastAPI app using best practices from research.md"

# Generate tests
claude --agent test-generator "Create tests for the FastAPI app"

# Review everything
claude --agent code-reviewer "Review all files in src/"
```
...
claude --agent backend-developer "Create API using $FRAMEWORK with $DATABASE"
```
...
claude --agent doc-generator "Write commit message for: $CHANGES"

# Pre-merge review
claude --agent security-auditor "Review changes in this branch"
```
...
claude --agent security-auditor "Review staged files for security issues"
...
review:
  ...
  steps:
    ...
    - name: Install Claude Code CLI
      run: Install Claude Code from the official Anthropic documentation
      
    - name: Security Review
      run: |
        claude --agent security-auditor "Review PR changes for vulnerabilities"
      
    - name: Code Quality Review
      run: |
        claude --agent code-reviewer "Review PR for code quality issues"
```
...
for file in src/**/*.py; do
  ...
  claude --agent python-expert "Add type hints to $file" > "${file}.tmp"
  mv "${file}.tmp" "$file"
done
...
claude --agent orchestrator "Build complete CI/CD pipeline"
```

**2. Code Reviewer**
```bash
# Automated code reviews
claude --agent code-reviewer "Review PR #123"
```

**3. Security Auditor**
```bash
# Security scanning in CI
claude --agent security-auditor "Scan codebase for vulnerabilities"
```

**4. Test Generator**
```bash
# Generate tests in batch
claude --agent test-generator "Create tests for all services in src/services/"
```
...
# Deployment
claude --agent docker-expert "Optimize Dockerfile"
```
...
claude --agent enhanced-planner "Plan migration to microservices" > plan.md
```
...
for file in src/**/*.ts; do
  claude --agent doc-generator "Add JSDoc to $file" > "${file}.tmp"
  mv "${file}.tmp" "$file"
done
```
...
claude --agent terraform-expert "Create VPC module"

# Container optimization
claude --agent docker-expert "Optimize production Dockerfile"
...
**Problem:**
```bash
claude --agent backend-developer "Update the user service"
# Error: Can't find user service
```

**Solution:** Be explicit with paths:
```bash
claude --agent backend-developer "Update the user service in src/services/user.service.ts"
```
...
```bash
claude --agent doc-generator "Create API docs" | tee docs.md
claude --agent orchestrator "Complex task" | less
```
