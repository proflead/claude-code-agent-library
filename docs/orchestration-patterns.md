...
```bash
claude --agent orchestrator "Build a REST API for a task management system"
```
...
```bash
claude --agent orchestrator "Build a user profile feature with avatar upload, bio editing, and activity timeline"
```
...
```bash
claude --agent orchestrator "Create a payment processing microservice with API, database, tests, and deployment configuration"
```
...
```bash
claude --agent orchestrator "Implement this design with optimization and accessibility"
```
...
```bash
claude --agent orchestrator "Build an e-commerce checkout flow with:
...
```bash
claude --agent orchestrator "Plan and execute migration from AngularJS to React while maintaining feature parity"
```
...
```bash
claude --agent orchestrator "Build a project management SaaS with:
- User authentication
...
```bash
# Step 1: Research
claude --agent research-agent "Research best practices for real-time chat apps"

# Step 2: Design (using research findings)
claude --agent api-designer "Design WebSocket API for real-time chat based on research"

# Step 3: Implement
claude --agent backend-developer "Implement WebSocket chat API per design"

# Step 4: Test
claude --agent e2e-tester "Create tests for real-time chat functionality"
```
...
claude --agent orchestrator "Build REST API for blog platform"

# Manual: You review and test

# Auto: Iterate based on feedback
claude --agent backend-developer "Add pagination to blog posts endpoint"
claude --agent test-generator "Add tests for pagination"
```
...
### 1. Start with Clear Requirements
 ...
 Bad:
```bash
claude --agent orchestrator "Build something for users"
```

 Good:
```bash
claude --agent orchestrator "Build a user authentication system with:
...
```bash
# Phase 1
claude --agent orchestrator "Set up project foundation with auth and database"

# Review output, then Phase 2
claude --agent orchestrator "Add core features: projects, tasks, comments"

# Review, then Phase 3
claude --agent orchestrator "Add real-time collaboration features"
```
...
```bash
claude --agent orchestrator "Build a REST API similar to GitHub's API:
