...
# Run code review
claude --agent code-reviewer "Review changes in this PR focusing on security, performance, and best practices"
```
...
```bash
claude --agent frontend-developer "Create a DataTable component with:
- Sorting by column
...
```bash
claude --agent api-designer "Design REST API for task management with:
- Projects and tasks
- User assignments
...
**CLI Example:**
```bash
claude --agent backend-developer "Create user authentication API with:
...
**CLI Example:**
```bash
claude --agent database-architect "Design PostgreSQL schema for social media app:
- Users with profiles
...
**CLI Example:**
```bash
claude --agent orchestrator "Build user notification system:
...
**CLI Example:**
```bash
claude --agent orchestrator "Build payment processing API with security focus:
- Stripe integration
- PCI DSS compliance
...
**Developer Request:**
```bash
claude --agent sql-expert "Optimize this user + posts query that's causing N+1 problem"
```
...
**Developer Request:**
```bash
claude --agent responsive-design "Make this dashboard responsive for mobile, tablet, desktop"
```
...
**Developer Request:**
```bash
claude --agent test-generator "Create comprehensive Jest tests for UserService with mocks"
```
...
claude --agent api-designer "Design user API"      
Output: openapi.yaml                                  
...
1. Write test (describe what code should do)
 claude --agent test-generator "Create tests for calculateDiscount function"

2. Run test (should fail - red)
 npm test

3. Implement feature
 claude --agent backend-developer "Implement calculateDiscount to pass tests"

4. Run test (should pass - green)
 npm test

5. Refactor
 claude --agent code-reviewer "Review calculateDiscount for improvements"
```
...
claude --agent code-reviewer "Review for obvious issues"
        
Fix identified issues
        
claude --agent security-auditor "Security scan with OWASP focus"
        
Address vulnerabilities
        
claude --agent test-generator "Create security-focused tests"
...
claude --agent enhanced-planner "Plan adding Google OAuth to our Express.js app with existing JWT auth"

# Step 2: Implement OAuth integration
claude --agent backend-developer "Implement Google OAuth with Passport.js, integrate with existing JWT system"

# Step 3: Update frontend
claude --agent frontend-developer "Add 'Sign in with Google' button and OAuth flow to React login page"

# Step 4: Test
claude --agent e2e-tester "Create Playwright tests for OAuth login flow"

# Step 5: Security review
claude --agent security-auditor "Review OAuth implementation for security issues"
```
...
claude --agent performance-optimizer "API /users/feed takes 3 seconds. Here's the code: [paste code]"

# Step 2: Optimize database queries
claude --agent sql-expert "Optimize this query: [paste slow query]"

# Step 3: Add caching
claude --agent backend-developer "Add Redis caching to user feed with 5-minute TTL"

# Step 4: Verify improvements
claude --agent test-generator "Create load tests to verify feed performance under 100ms"
```
...
claude --agent legacy-modernizer "Create migration plan from AngularJS to React for our admin dashboard"

# Step 2: Create migration strategy
claude --agent enhanced-planner "Break down migration into phases with rollback plan"

# Step 3: Start with routing
claude --agent frontend-developer "Create React routing to match existing AngularJS routes"

# Step 4: Component migration (iterative)
claude --agent frontend-developer "Convert UserList component from AngularJS to React with TypeScript"

# Step 5: Testing parallel
claude --agent e2e-tester "Create tests ensuring React version matches AngularJS behavior"
```
